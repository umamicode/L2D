...
from utils.barlowtwins import BarlowTwinsLoss
...
def get_args():
    ...
    parser.add_argument("--loss_fn", default='supcon', type=str, help="supcon/barlowtwins")
...
class Trainer:
    def __init__(self, args, device):
        ...
        self.flag = False
        if self.args.loss_fn =='supcon':
            self.con = SupConLoss()
        elif self.args.loss_fn == 'barlowtwins':
            self.con = BarlowTwinsLoss(projection_dim=512)
    ...
    def _do_epoch(self, epoch=None):
        ...
        for it, ((data, _, class_l), _, idx) in enumerate(self.source_loader):
            ...
            if self.args.loss_fn == 'supcon':
                ...
                #projection_dim= list(emb_src.shape)[-1]
                #self.con.projection_dim= projection_dim
                con = self.con(torch.cat([emb_src, emb_aug], dim=1), adv=False)
            # Likelihood
    ...
    def do_training(self):
        ...
        for self.current_epoch in range(self.args.epochs):
            ...
            if self.results["test"][self.current_epoch] > current_high:
                ...
                svpath= os.path.join('/home/dongkyu/Learning_to_diversify/logs/model/', 'best_'+self.args.target[0]+'.pkl')
                torch.save(self.extractor.state_dict(), svpath )
...
def main():
    args = get_args()
    print(args)
    print("Training with: ", args.loss_fn)
    if args.task == 'PACS':
        args.n_classes = 7
